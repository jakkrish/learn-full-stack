...
function passwordProtected(req,res,next){
    res.set("WWW-Authenticate",'Basic realm="Simple To do app"'); //admin,admin
...
myServer.use(passwordProtected);
...
myServer.post("/add-value",function(req,res){
    ...
    if(!(req.body.text=="")){
        db.collection('todo').insertOne({item:safeText},function(err,info){
            res.json(info.ops[0]);
            // res.redirect("/");
        });
    }
...
myServer.post("/edit-value",function(req,res){
    let safeText = sanitizeHTML(req.body.text, {allowedTags:[],allowedAttributes:{}});
    if(!(req.body.text=="")){
        db.collection('todo').findOneAndUpdate({_id: new mongodb.ObjectID(req.body.id)},{$set:{item:safeText}},function(){
            res.send('success');
        });
    }
});
